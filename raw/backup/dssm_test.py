...
def load_train_data(query_path, doc_path):
    ...
    query_train_data = np.asarray(get_onehot_vec(query_path), dtype=np.float32)
    doc_train_data = np.asarray(get_onehot_vec(doc_path), dtype=np.float32)
...
with tf.Session(config=config) as sess:
    ...
    ls = sess.run(loss, feed_dict={query_batch: query_test_data, doc_batch: doc_test_data})
    acc = sess.run(accuracy, feed_dict={query_batch: query_test_data, doc_batch: doc_test_data})
    print('loss: %f accuracy: %f' % (ls, acc))
