...
flags.DEFINE_string('summaries_dir', '/tmp/dssm-v4', 'Summaries directory')
...
flags.DEFINE_string('model_version', 'v4', 'model version')
...
with tf.device(device_name):
    ...
    with tf.name_scope('loss'):
        weight = tf.constant(np.array([[1,0],[0,SAMPLE-1]], dtype=np.float32), dtype=tf.float32)
        weighted_label = tf.matmul(label, weight)
        ...
        loss = tf.reduce_mean(tf.reduce_sum(tf.nn.softmax_cross_entropy_with_logits(logits=out, labels=weighted_label)) / (tf.cast(tf.shape(pred), dtype=tf.float32)/SAMPLE*(SAMPLE-1)*2))
...
with tf.Session(config=config) as sess:
    ...
    for epoch in range(FLAGS.max_epoch):
        for step in range(epoch_steps):
            ...
            mgd, ls, acc, pred_, label_, l_rate, out_stmx = sess.run([merged, loss,accuracy,pred_y, label_y, learning_rate, out_softmax], feed_dict={query: query_batch, doc:doc_batch, label:label_batch})
            ...
            if len(previous_losses) >= 5 and ls > max(previous_losses[-5:]):
                ...
                print("Learning rate decay Epoch %d, Step %d , learning rate %f" % (epoch+1, step+1, learning_rate.eval()))
