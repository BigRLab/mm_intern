...
with tf.device(device_name):
    ...
    with tf.name_scope('mlp_out'):
        out_par_range = np.sqrt(6.0 / (L3_N + 1))
        wo1 = tf.Variable(tf.random_uniform([L3_N,1], -out_par_range, out_par_range), 'wo1')
...
#        out_norm = tf.nn.l2_normalize(out, 1)
    ...
    with tf.name_scope('loss'):
        loss = tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(logits=out, labels=input_label))
        pred = tf.equal(tf.argmax(tf.nn.sigmoid(out), 1), tf.argmax(input_label, 1))
...
with tf.Session(config=config) as sess:
    if os.path.exists('../model/dssm_v2.ckpt.index'):
        saver.restore(sess, '../model/dssm_v2.ckpt')
    else:
    ...
    for epoch in range(FLAGS.max_epoch):
        for step in range(FLAGS.epoch_steps):
            ...
            print(sess.run(tf.nn.sigmoid(out), feed_dict={input_batch: batch_data, input_label:batch_label}))
            exit()
            ...
            if ls < max_loss:
                saver_path = saver.save(sess, "../model/dssm_v2.ckpt")
