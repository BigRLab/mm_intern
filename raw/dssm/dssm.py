    ...
    def __init__(self, sample = 11, sample_nums = 200, lr = 0.1, lrdf = 0.95, l1 = 1000, l2 = 240, l3 = 400, cks = [2,3,4], ckn = [128, 64, 64]):
        ...
        with tf.device(device_name):
            ...
            with tf.name_scope('w2v'):
                self.words = tf.Variable(self.__load_w2v('../model/vec.txt', 900), dtype=tf.float32, name='words')
            ...
            with tf.name_scope('convolution_layer'):
                ...
                for i, size in enumerate(self.conv_kernel_size):
                    ...
                    self.doc_conv[size] = self.__conv2d('conv' + str(size), self.query_words_out, self.wc[size], self.bc[size])
                    self.doc_pool[size] = self.__full_max_pool('pool' + str(size), self.query_conv[size], [0, 3, 2, 1])
