...
ep_steps = len(input_data)/(BS*(NEG+2))
flags.DEFINE_integer('epoch_steps', ep_steps, 'epoch step')
...
with tf.name_scope('loss'):
    ...
    np_y[:,0] += 1
    y_ = tf.constant(np_y, dtype=tf.float32)
    loss = tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(logits=out_norm, labels=y_))
...
with tf.Session(config=config) as sess:
    ...
    for epoch in range(FLAGS.max_epoch):
        for step in range(30):
            batch_data = get_batch_data(step)
            acc = sess.run(accuracy, feed_dict={input_batch: batch_data})
            sess.run(train_step, feed_dict={input_batch: batch_data})
            ls = sess.run(loss, feed_dict={input_batch: batch_data})
