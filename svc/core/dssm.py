...
w2v_path = os.path.join(os.path.split(os.path.realpath(__file__))[0], '../model/vec.txt')
...
class Model(object):
    ...
    def __init__(self, sample = 11, sample_nums = 200, lr = 0.1, lrdf = 0.95, l1 = 1000, l2 = 240, l3 = 400, cks = [2,3,4], ckn = [128, 64, 64]):
        ...
        with tf.device(device_name):
            ...
            with tf.name_scope('w2v'):
                self.words = tf.Variable(self.__load_w2v(w2v_path, FLAGS.embedding_dim), dtype=tf.float32, name='words')
    ...
    def load_model(self, sess, model_path):
        ...
        if os.path.exists(model_path + '.index'):
            ...
            print "load dssm model params"
        else:
            sess.run(tf.global_variables_initializer())
            print "init dssm model params"
